...
def build_verdict_prompt(own_card: dict, met_rec: dict) -> List[dict]:
    """
    Builds a strict JSON-only prompt for Stage J.
    """
    ...
    sys = (
        ...
        "- rationale: 3–6 коротких пунктов.\n"
...
def build_exec_summary_prompt(payload: dict) -> List[dict]:
    system = (
        f"Ты пишешь Executive Summary по результатам ревайва {payload.get('sku_count') or 'N'} SKU на Wildberries. "
        "У тебя есть агрегированные цифры и примеры nm_id по корзинам. "
        ...
        "  \"watchouts\": [string, ...]\n"
        "}\n"
        "Ограничения: headline <= 140 символов; overall <= 600 символов; "
        "в списках по 3-7 пунктов. "

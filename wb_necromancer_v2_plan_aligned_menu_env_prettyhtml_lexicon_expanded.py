...
# =========================
# Minimal .env loader (like wb_revive.py)
# - Loads .env from script dir and cwd (does not override existing env by default)
# - Ensures argparse defaults can see env values early
# =========================
...
def load_env_file(env_path: Path, override: bool = False) -> None:
    try:
        ...
        # .env is best-effort; don't brick the run because humans can't type KEY=VALUE
        return
...
def write_dotenv(env_path: Path, updates: Dict[str, str]) -> None:
    # Preserve existing keys, update/insert new, keep comments/unknown lines
    existing = []
    try:
        if env_path.exists():
...
# Load .env early so argparse defaults can see it.
try:
...
DEFAULT_PULSE_RULES = {
    # Rules used by Stage K to classify ALIVE/SLOW/DEAD from Market Pulse.
    # You can override these in run_manifest.json -> config -> pulse_rules.
...
DEFAULT_SUPPLY_THRESHOLDS = {
    # Stage J thresholds (override via manifest config -> supply_thresholds)
    "min_n": 8,
    "dumping": {
    ...
    *,
    ...
    manifest = {
        ...
        "config": {
            ...
            "pulse_rules": dict(DEFAULT_PULSE_RULES),
            "supply_thresholds": dict(DEFAULT_SUPPLY_THRESHOLDS),
            "filters": fcfg,
        },
        "paths": {
...
def stage_D_query_build(
    ...
    *,
    ...
    llm_extra_per_cluster: int = 3,
    min_len: int = 8,
    max_len: int = 64,
    # Optional LLM enrichment (plan: rules + LLM)
    ...
    Rules-first + optional LLM enrichment (strict re-validation + fallback).
    """
    manifest_path = out_dir / "run_manifest.json"
    ...
    def _llm_suggest_queries(
        *,
        ...
        """
        English technical:
        Ask LLM for extra WB search queries. Must output JSON: {"queries":[...]}.
        We ALWAYS re-validate via _validate_query and dedupe.
        """
        if not use_llm_d or want_n <= 0 or not phone_model.strip():
            return [], {"used": False}

        # Keep prompt small and deterministic.
        crit = []
        if cluster == "phone":
            crit.append("Must be a search query for phone cases for the given phone model.")
            crit.append("Should include the phone model tokens (at least half of them).")
        else:
            crit.append("Must be a search query for TPU/silicone phone case WITH card pocket/cardholder for the given phone model.")
            crit.append("Must include pocket/cardholder intent and TPU/silicone intent.")

        # Expand must_any_groups into compact text for the model.
        group_lines = []
        for g in must_any_groups or []:
            name = str(g.get("name") or "group")
            terms = [str(x) for x in (g.get("any") or []) if str(x).strip()]
            if terms:
                group_lines.append(f"- {name}: any of {terms[:8]}")
        groups_txt = "\n".join(group_lines) if group_lines else "(none)"

        system = (
            "You generate Russian Wildberries search queries. "
            "Return ONLY valid JSON object with key 'queries' (array of strings). "
        ...
        user = (
            ...
            f"CLUSTER: {cluster}\n"
            f"GOAL: Generate {int(want_n)} additional search queries.\n\n"
            f"CRITERIA:\n- " + "\n- ".join(crit) + "\n"
            f"- Length {int(min_len)}..{int(max_len)} characters.\n"
            f"- Do NOT include banned terms: {ban_terms[:30]}\n"
            f"- Must satisfy must-any groups:\n{groups_txt}\n"
            f"- Avoid duplicates of existing queries: {existing[:12]}\n\n"
            "OUTPUT JSON schema:\n"
            '{"queries":["...","..."]}\n'
        )
        ...
        dbg = {"used": True, "provider": llm_provider, "model": llm_model}
        try:
            resp, ddbg = call_llm_json(
            ...
            dbg.update({k: ddbg.get(k) for k in ["status", "latency_s", "url"] if k in ddbg})
        except Exception as e:
            dbg["error"] = repr(e)
            return [], dbg
        ...
        kept = _dedupe_keep_order(kept)
        kept = [q for q in kept if q not in set(existing)]
        kept = kept[: int(want_n)]
        dbg["returned"] = len(qs)
        dbg["kept"] = len(kept)
        dbg["dropped_invalid"] = int(dropped)
        return kept, dbg
            ...
            for cluster in ("phone", "type"):
                ...
                for q in rules_qs:
                    ...
                    if ok:
                        ...
                        dropped_rules.append({"q": q, "reasons": why})

                valid_rules = _dedupe_keep_order(valid_rules)
                valid_rules = _take_top_n(valid_rules, rules_per_cluster)

                llm_qs: List[str] = []
                llm_dbg: Dict[str, Any] = {"used": False}
                if use_llm_d and llm_extra_per_cluster and phone_model:
                    llm_qs, llm_dbg = _llm_suggest_queries(
                        cluster=cluster,
                        phone_model=phone_model,
                        want_n=int(llm_extra_per_cluster),
                ...
                packs.append({
                    ...
                    "dropped_rules": dropped_rules[:20],
                    "must_any_groups": must_any_groups,
                    "ban_terms": ban_terms,
                    "llm": llm_dbg,
                })
...
CASE_INTENT_TERMS = ["чехол", "чехл", "case", "cover", "кейс", "наклад", "книжк", "бампер", "футляр", "обложк", "флип", "flip", "book", "панел"]
POCKET_HINT_TERMS = ["карман", "кармашк", "картхолд", "картхолдер", "cardhold", "cardholder", "держатель карт", "держател", "для карт", "для карты", "под карту", "под карт", "карта", "карты", "карту", "карте", "картой", "карточк", "кошел", "кошелек", "кошельк", "wallet", "портмон", "слот", "отсек", "card slot", "card-slot"]
TPU_HINT_TERMS = ["tpu", "тпу", "soft tpu", "силикон", "силиконов", "силиконовый", "термополиуретан", "полиуретан", "термопластич"]
...
# --- optional external lexicon (manual overrides) ---
# If a file named `necromancer_lexicon.json` exists (next to the script or in CWD),
# we merge extra terms into the built-in term lists. This lets you tweak словари вручную
# without touching the code.
def _load_external_lexicon() -> None:
    global CASE_INTENT_TERMS, POCKET_HINT_TERMS, TPU_HINT_TERMS

    def _dedupe(seq):
        seen = set()
        out = []
        for it in seq:
            k = str(it).strip().lower().replace("ё", "е")
            if not k or k in seen:
                continue
            seen.add(k)
            out.append(str(it))
    ...
    candidates = []
    try:
        candidates.append(Path(__file__).with_name("necromancer_lexicon.json"))
    except Exception:
        pass
    try:
        candidates.append(Path.cwd() / "necromancer_lexicon.json")
    except Exception:
        pass
    ...
    for p in candidates:
        try:
            ...
            if not isinstance(data, dict):
                continue
            CASE_INTENT_TERMS = _dedupe(CASE_INTENT_TERMS + list(data.get("case_terms") or []))
            POCKET_HINT_TERMS = _dedupe(POCKET_HINT_TERMS + list(data.get("pocket_terms") or []))
            TPU_HINT_TERMS = _dedupe(TPU_HINT_TERMS + list(data.get("tpu_terms") or []))
            print(f"[LEXICON] Loaded: {p}")
            break
        except Exception as e:
            print(f"[LEXICON] Failed to load {p}: {e}")

_load_external_lexicon()
...
def stage_H_relevance_filter(
    ...
    *,
    ...
    max_per_seller: int = 3,
    # Optional LLM for borderline cases (plan)
    ...
    filters_cfg = ((manifest.get("config") or {}).get("filters") or {})
    pocket_terms = list(filters_cfg.get("pocket_terms") or []) or POCKET_HINT_TERMS
    tpu_terms = list(filters_cfg.get("tpu_terms") or []) or TPU_HINT_TERMS
    ...
    # borderline reason allowlist (we won't ask LLM for obviously wrong stuff)
    BORDERLINE_ALLOW = {
        "missing_group:model_tokens",
        "missing_group:model",
        "missing_group:phone_model",
        "no_tpu_signal",
        "no_pocket_signal",
    }

    def _is_borderline(reasons: List[str]) -> bool:
        if not reasons:
            return False
        # never borderline if it's not even a case intent
        if "no_case_intent" in reasons:
            return False
        return all((r in BORDERLINE_ALLOW) or r.startswith("missing_group:") for r in reasons)

    def _llm_decide_keep(
        *,
        cluster: str,
        phone_model: str,
        items: List[dict],
        ...
        """
        English technical:
        Batch ask LLM to classify borderline items.
        Output JSON:
          {"decisions":[{"nm_id":"...","keep":true/false,"why":"..."}]}
        """
        if not use_llm_h or not items:
            return {}, {"used": False}

        system = (
            "You are a strict classifier for Wildberries product relevance. "
            "Return ONLY valid JSON. No markdown."
        )

        if cluster == "phone":
            crit = (
                "KEEP only if the item is a phone case/cover for the given phone model. "
                "DROP screen protectors, cables, chargers, straps, glass/film, and unrelated accessories."
            )
        else:
            crit = (
                "KEEP only if the item is a TPU/silicone phone case AND has a card pocket/cardholder/slot for cards "
                "for the given phone model. DROP anything without card pocket intent."
            )
        ...
        for it in items:
            payload_items.append({
                ...
                "name": str(it.get("name") or ""),
                "subject": str(it.get("subject_name") or ""),
            })
        ...
        user = (
            ...
            f"CLUSTER: {cluster}\n"
            f"CRITERIA: {crit}\n"
            f"BANNED_TERMS (already filtered, but still don't KEEP if you see them): {ban_terms[:30]}\n\n"
            f"ITEMS:\n{json.dumps(payload_items, ensure_ascii=False)}\n\n"
            'OUTPUT JSON schema: {"decisions":[{"nm_id":"...","keep":true/false,"why":"short"}]}\n'
        )

        messages = [{"role": "system", "content": system}, {"role": "user", "content": user}]

        dbg = {"used": True, "provider": llm_provider, "model": llm_model, "count": len(items)}
        try:
            resp, ddbg = call_llm_json(
                provider=llm_provider,
                model=llm_model,
            ...
            if isinstance(decs, list):
                for d in decs:
                    if not isinstance(d, dict):
                        continue
                    nm = str(d.get("nm_id") or "").strip()
                    if not nm:
                        continue
                    keep = bool(d.get("keep"))
                    keep_map[nm] = keep
        dbg["kept_n"] = sum(1 for v in keep_map.values() if v)
        dbg["seen_n"] = len(keep_map)
        return keep_map, dbg
        ...
        for line in f:
            ...
            for cluster in ("phone", "type"):
                ...
                for it in merged:
                    ...
                    if not ok:
                        # keep as borderline candidate if it's plausibly close
                        if _is_borderline(why):
                            borderline.append({**it, "_drop_reasons": why})
                        else:
                ...
                # Optional LLM rescue for borderline
                llm_dbg = {"used": False}
                if use_llm_h and borderline and len(kept) < int(max_keep_per_cluster) and phone_model:
                    # Take a small batch; keep prompt cost bounded
                    cand = borderline[: int(borderline_cap)]
                    keep_map, llm_dbg = _llm_decide_keep(
                        cluster=cluster,
                        phone_model=phone_model,
                        items=cand,
                        ban_terms=ban_terms,
                    ...
                    # Apply keeps with conservative gates and seller caps
                    added = 0
                    for it in cand:
                        nm_id = str(it.get("nm_id") or "")
                        if not nm_id or not keep_map.get(nm_id, False):
                            dropped.append({"nm_id": nm_id, "reason": ["llm_drop"] + list(it.get("_drop_reasons") or [])})
                            continue
                        ...
                        # Hard minimal gates (we still don't trust LLM completely)
                        if not _contains_any(text, CASE_INTENT_TERMS):
                            dropped.append({"nm_id": nm_id, "reason": ["llm_keep_rejected:no_case_intent"]})
                            continue
                        if cluster == "type":
                            # Pocket intent is non-negotiable for type-market
                            if not _contains_any(text, pocket_terms):
                                dropped.append({"nm_id": nm_id, "reason": ["llm_keep_rejected:no_pocket_intent"]})
                                continue
                            # TPU terms are desirable but can be implicit; do not hard-reject on TPU.
                        ...
                        kept.append(it)
                        added += 1
                        if len(kept) >= int(max_keep_per_cluster):
                            break

                    if llm_dbg.get("used"):
                        notes.append(f"LLM_BORDERLINE_{cluster}:added={added}")
                ...
                out_clusters.append({
                    ...
                    "stats": {
                        ...
                        "kept": len(kept_min),
                        "dropped": len(dropped),
                        "borderline_seen": len(borderline),
                    },
                    "llm": llm_dbg,
                })
...
def stage_J_supply_structure(
    ...
    *,
    ...
    cfg = manifest.get("config") or {}
    sth = cfg.get("supply_thresholds") or {}
    # Allow manifest to override thresholds without code edits
    min_n_eff = int(sth.get("min_n") or min_n)

    dump = sth.get("dumping") or {}
    dump_min_unique_sellers = int(dump.get("min_unique_sellers") or 10)
    dump_p10_ratio_lt = float(dump.get("p10_ratio_lt") or 0.75)
    dump_outlier_rate_gte = float(dump.get("outlier_rate_gte") or 0.10)
    dump_min_price_n_or = int(dump.get("min_price_n_or") or 12)

    mono = sth.get("monopoly") or {}
    mono_top1_share_gte = float(mono.get("top1_share_gte") or 0.35)
    mono_hhi_gte = float(mono.get("hhi_gte") or 0.25)
    ...
    with rel_path.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            for cl in ("phone", "type"):
                ...
                # Confidence
                if len(prices) < min_n_eff or len(kept) < min_n_eff:
                    flags.append("LOW_CONFIDENCE")
                # Dumping pressure: cheap tail vs median + many sellers
                if p_med and p10 and unique_sellers >= dump_min_unique_sellers:
                    if p10 < dump_p10_ratio_lt * p_med and (out_rate >= dump_outlier_rate_gte or len(prices) >= dump_min_price_n_or):
                        flags.append("DUMPING_PRESSURE")
                # Monopoly danger: seller concentration (counts)
                if top1_share is not None and top1_share >= mono_top1_share_gte:
                    flags.append("MONOPOLY_DANGER")
                if hhi is not None and hhi >= mono_hhi_gte:
...
def _pulse_to_status(cluster_row: dict, rules: Dict[str, Any]) -> Tuple[str, str, List[str]]:
    """
    English technical:
    ...
    if r30 >= alive_r30_med_gte:
        return "ALIVE", conf, flags
    if dmin is not None and dmin <= alive_days_since_last_min_lte:
        return "ALIVE", conf, flags
    if r90 >= alive_r90_med_gte and (dmed is None or dmed <= alive_days_since_last_med_lte):
        return "ALIVE", conf, flags

    # Slow market signals
    if r90 >= slow_r90_med_gte and (dmed is None or dmed <= slow_days_since_last_med_lte):
        return "SLOW", conf, flags
    if dmin is not None and dmin <= slow_days_since_last_min_lte and r90 > 0:
        return "SLOW", conf, flags
...
def stage_K_cluster_verdicts(
    ...
    *,
    ...
    cfg = manifest.get("config") or {}
    pulse_rules = cfg.get("pulse_rules") or {}
    if not isinstance(pulse_rules, dict):
        pulse_rules = {}
    # Backward compatible defaults if manifest doesn't have pulse_rules yet
    if not pulse_rules:
        pulse_rules = dict(DEFAULT_PULSE_RULES)
    ...
    for own_nm, mp in mp_map.items():
        ...
        for cl in ("phone", "type"):
            ...
            status, conf, pulse_flags = _pulse_to_status(mp_row, pulse_rules)
...
def _write_html_pretty(html_path: Path, rows: List[dict], meta: dict, exec_summary: Optional[dict] = None) -> None:
    """Self-contained 'pretty' HTML like wb_revive.py (filters + collapsible details).

    RU:
    - Без внешних библиотек.
    - Поиск + фильтр по решению + чекбоксы по флагам.
    - Детали в <details>.
    """
    ...
    def badge(verdict: str) -> str:
        v = (verdict or "").upper()
        cls = {"DROP":"b-drop","REVIVE_FAST":"b-fast","REVIVE_REWORK":"b-rework","CLONE_NEW_CARD":"b-clone"}.get(v, "b-unk")
        ru = {"DROP":"DROP","REVIVE_FAST":"REVIVE_FAST","REVIVE_REWORK":"REVIVE_REWORK","CLONE_NEW_CARD":"CLONE_NEW_CARD"}.get(v, v)
        return f"<span class='badge {cls}' title='{_h(v)}'>{_h(ru)}</span>"

    def is_flag(flags_str: str, key: str) -> str:
        return "1" if key in (flags_str or "") else "0"
    ...
    # ---- header + styles ----
    title = "WB Revival v2 Necromancer"
    gen_at = _h((meta or {}).get("generated_at_utc") or "")
    run_id = _h((meta or {}).get("run_id") or "")
...
header{position:sticky;top:0;z-index:50;background:rgba(247,248,251,.92);backdrop-filter: blur(8px); border-bottom:1px solid var(--border);}
...
@media(max-width:1000px){.cards{grid-template-columns:repeat(2,minmax(140px,1fr));}}
.badge{display:inline-flex;align-items:center;gap:6px;border-radius:999px;padding:4px 10px;font-size:12px;font-weight:800;border:1px solid transparent;white-space:nowrap}
...
.b-unk{background:rgba(100,116,139,.10);color:var(--muted);border-color:rgba(100,116,139,.22)}
...
"""
    ...
    cards = f"""
<div class='cards'>
  <div class='card'><div class='k'>Всего SKU</div><div class='v info'>{total}</div><div class='k mono'>run_id</div><div class='muted mono'>{run_id}</div></div>
  <div class='card'><div class='k'>REVIVE_FAST</div><div class='v ok'>{int(counts.get('REVIVE_FAST',0) or 0)}</div></div>
  <div class='card'><div class='k'>REVIVE_REWORK</div><div class='v warn'>{int(counts.get('REVIVE_REWORK',0) or 0)}</div></div>
  <div class='card'><div class='k'>CLONE_NEW_CARD</div><div class='v info'>{int(counts.get('CLONE_NEW_CARD',0) or 0)}</div></div>
  <div class='card'><div class='k'>DROP</div><div class='v bad'>{int(counts.get('DROP',0) or 0)}</div></div>
  <div class='card'><div class='k'>generated_at_utc</div><div class='v info' style='font-size:14px'>{gen_at}</div></div>
</div>
"""
    ...
    if exec_summary and isinstance(exec_summary, dict):
        exec_panel = (
            "<details class='panel' style='margin:12px 0'><summary style='cursor:pointer;font-weight:800;color:var(--accent)'>Exec summary (JSON)</summary>"
            "<div class='box mono' style='white-space:pre-wrap;margin-top:8px'>" + _h(json.dumps(exec_summary, ensure_ascii=False, indent=2)) + "</div></details>"
        )
    ...
    for r in rows:
        ...
        vc = str(r.get('vendor_code') or '')
        name = str(r.get('name') or '')
        verdict = str(r.get('final_decision') or '')
        flags = str(r.get('risk_flags') or '')

        phone_status = str(r.get('phone_status') or '')
        type_status = str(r.get('type_status') or '')
        phone_conf = str(r.get('phone_conf') or '')
        type_conf = str(r.get('type_conf') or '')

        wb_url = f"https://www.wildberries.ru/catalog/{_h(nm)}/detail.aspx" if nm else "#"

        first_task = ""
        bl = str(r.get('backlog_ru') or '').strip()
        if bl:
            first_task = bl.split('|')[0].strip()
        ...
        ds = {
            ...
            "low": is_flag(flags, "LOW_CONFIDENCE"),
            "dump": is_flag(flags, "DUMPING_PRESSURE"),
            "mono": is_flag(flags, "MONOPOLY_DANGER"),
            "deadp": "1" if phone_status == "DEAD" else "0",
            "deadt": "1" if type_status == "DEAD" else "0",
        }
        ds_attr = " ".join([f"data-{k}='{_h(v)}'" for k, v in ds.items()])

        details = (
            "<details class='details'><summary>показать</summary>"
            "<div class='box' style='margin-top:10px'>"
            f"<div class='muted'><b>Факты:</b> phone_price_med={_h(fmt(r.get('phone_price_med')))} | type_price_med={_h(fmt(r.get('type_price_med')))} | "
            f"phone_r30_med={_h(fmt(r.get('phone_recent30_med')))} | type_r30_med={_h(fmt(r.get('type_recent30_med')))}</div>"
            f"<div style='margin-top:8px'><b>Флаги:</b> <span class='mono'>{_h(flags)}</span></div>"
            f"<div style='margin-top:8px'><b>Rationale:</b><div class='mono' style='white-space:pre-wrap'>{_h(r.get('rationale_ru') or '')}</div></div>"
            f"<div style='margin-top:8px'><b>Backlog:</b><div class='mono' style='white-space:pre-wrap'>{_h(bl)}</div></div>"
            f"<div style='margin-top:8px'><b>Alt strategy:</b><div class='mono' style='white-space:pre-wrap'>{_h(r.get('alt_strategy_ru') or '')}</div></div>"
            "</div></details>"
        )

        row_html.append(f"""
<tr class='row' {ds_attr}>
  <td class='mono'>
    <div><a href='{wb_url}' target='_blank' rel='noreferrer'>{_h(nm)}</a></div>
    <div class='muted mono'>{_h(vc)}</div>
  </td>
  <td>
    <div class='name'>{_h(name) if name else '<span class=muted>—</span>'}</div>
  </td>
  <td>{badge(verdict)}</td>
  <td class='mono'>phone: {_h(phone_status)} <span class='muted'>({_h(phone_conf)})</span><br/>type: {_h(type_status)} <span class='muted'>({_h(type_conf)})</span></td>
  <td class='mono'>phone ₽{_h(fmt(r.get('phone_price_med')))}<br/>type ₽{_h(fmt(r.get('type_price_med')))}</td>
  <td class='mono'>phone r30={_h(fmt(r.get('phone_recent30_med')))}<br/>type r30={_h(fmt(r.get('type_recent30_med')))}</td>
  <td class='mono'>{_h(flags) if flags else '—'}</td>
  <td>{_h(first_task) if first_task else '<span class=muted>—</span>'}</td>
  <td>{details}</td>
</tr>
""")
...
function applyFilters(){
  ...
  const fDump = document.getElementById('fDump').checked;
  const fMono = document.getElementById('fMono').checked;
  const fDeadP = document.getElementById('fDeadP').checked;
  const fDeadT = document.getElementById('fDeadT').checked;
  ...
  for (const r of rows){
    ...
    if (term){
      const hay = (r.dataset.nm + ' ' + r.dataset.name).toLowerCase();
      ok = hay.includes(term);
    }
    if (ok && v !== 'ALL') ok = (r.dataset.verdict === v);
    if (ok && fLow) ok = (r.dataset.low === '1');
    if (ok && fDump) ok = (r.dataset.dump === '1');
    if (ok && fMono) ok = (r.dataset.mono === '1');
    if (ok && fDeadP) ok = (r.dataset.deadp === '1');
    if (ok && fDeadT) ok = (r.dataset.deadt === '1');
...
function toggleDetails(open){
  document.querySelectorAll('td details').forEach(d => d.open = open);
}

document.getElementById('q').addEventListener('input', applyFilters);
document.getElementById('verdict').addEventListener('change', applyFilters);
['fLow','fDump','fMono','fDeadP','fDeadT'].forEach(id => {
...
<header>
  <div class='header-inner'>
    <div>
      <h1>{_h(title)}</h1>
      <div class='meta'>Показано: <span id='shown'>0</span> / {total} • run_id: <span class='mono'>{run_id}</span></div>
    </div>
    <div class='meta'>Два рынка: phone=общий спрос по модели, type=TPU+карман. Решение из Stage L.</div>
  </div>
</header>
...
<div class='container'>
  ...
  <div class='controls'>
    ...
    <label class='chk'><input type='checkbox' id='fLow'> LOW_CONFIDENCE</label>
    <label class='chk'><input type='checkbox' id='fDump'> DUMPING_PRESSURE</label>
    <label class='chk'><input type='checkbox' id='fMono'> MONOPOLY_DANGER</label>
    <label class='chk'><input type='checkbox' id='fDeadP'> phone DEAD</label>
    <label class='chk'><input type='checkbox' id='fDeadT'> type DEAD</label>

    <button id='openAll' title='Развернуть все детали'>Развернуть</button>
    <button id='closeAll' title='Свернуть все детали'>Свернуть</button>
  </div>
  ...
  <table>
    <thead>
      <tr>
        ...
        <th style='width:240px'>Рынки</th>
        <th style='width:140px'>Цена (median)</th>
        <th style='width:140px'>Пульс (r30 med)</th>
        <th style='width:240px'>Risk flags</th>
        <th style='width:260px'>Первое действие</th>
        <th style='width:120px'>Детали</th>
      </tr>
    </thead>
    <tbody>
      {''.join(row_html)}
  ...
  <div class='footer'>Подсказка: тут нет магии. Только факты из JSONL и немного фронтенда, чтобы глаза не вытекали.</div>
</div>
...
def stage_M_reports(
    ...
    *,
    ...
    # Map nm_id -> product name from own_norm.jsonl (if exists)
    own_name = {}
    own_path = out_dir / 'own_norm.jsonl'
    if own_path.exists():
        try:
    ...
    _write_html_pretty(report_html, rows, meta_out, exec_summary=exec_summary)
...
# =========================
# LLM helper (OpenAI/OpenRouter chat-completions)
# =========================

def _extract_first_content(resp_json: Any) -> str:
    """English technical: return assistant content from chat-completions like payload."""
    ...
    try:
        ...
        pass
    # OpenAI Responses API (best-effort)
    try:
        out = resp_json.get("output")
        if isinstance(out, list) and out:
            # Find first message content
            for item in out:
...
def call_llm_json(
    *,
    ...
    """
    English technical:
    - Sends chat-completions request to OpenAI or OpenRouter.
    - Returns (parsed_json_or_raw_text, debug_dict).
    """
    ...
    if not api_key:
        raise RuntimeError("LLM api_key is empty. Pass --llm_api_key or set env (OPENAI_API_KEY / OPENROUTER_API_KEY).")

    if base_url and base_url.strip():
        base = base_url.strip().rstrip("/")
    else:
        base = "https://api.openai.com/v1" if prov == "openai" else "https://openrouter.ai/api/v1"

    # Prefer chat-completions for compatibility with OpenRouter and ST backends.
    ...
    if prov == "openrouter":
        # Optional but nice-to-have; OpenRouter uses these for analytics.
        headers.setdefault("HTTP-Referer", "https://localhost")
        headers.setdefault("X-Title", "wb-necromancer")
    ...
    if force_json:
        # Most providers accept response_format for JSON mode, but some ignore it.
        payload["response_format"] = {"type": "json_object"}

    # Requests import is scattered in this file (humans...). Make it explicit here.
    import requests  # type: ignore

    t0 = time.time()
    r = requests.post(url, headers=headers, data=json.dumps(payload), timeout=int(timeout_sec))
    ...
    # Non-2xx => keep response text for debugging
    if r.status_code < 200 or r.status_code >= 300:
        dbg["error_text_snip"] = (r.text or "")[:800]
        raise RuntimeError(f"LLM HTTP {r.status_code}: {dbg.get('error_text_snip')}")

    js = r.json()
    txt = _extract_first_content(js).strip()
    dbg["raw_text_snip"] = txt[:500]

    if not force_json:
        return txt, dbg

    # Parse JSON, with a cheap salvage attempt (strip code fences)
    try:
        return json.loads(txt), dbg
    except Exception:
        cleaned = txt
        cleaned = cleaned.replace("```json", "").replace("```", "").strip()
        try:
            return json.loads(cleaned), dbg
        except Exception:
            # Return raw text so pipeline can still proceed deterministically
            return {"_parse_error": True, "_raw": txt[:5000]}, dbg
...
STAGE_INFO = {
    ...
    "D": {"title":"QUERY BUILD (rules + optional LLM enrichment)", "network":"LOCAL", "llm_capable": True},
    "E": {"title":"SERP SNAPSHOT + VALIDATION", "network":"WB", "llm_capable": False},
    "F": {"title":"COMPETITOR POOL", "network":"LOCAL", "llm_capable": False},
    "G": {"title":"COMPETITOR LITE FETCH", "network":"WB", "llm_capable": False},
    "H": {"title":"RELEVANCE FILTER (rules + optional LLM borderline)", "network":"LOCAL", "llm_capable": True},
    "I": {"title":"MARKET PULSE (reviews)", "network":"WB", "llm_capable": False},
    "J": {"title":"SUPPLY/STRUCTURE (WB facts, local calc)", "network":"LOCAL", "llm_capable": False},
    "K": {"title":"CLUSTER VERDICTS", "network":"LOCAL", "llm_capable": False},
    "L": {"title":"FINAL DECISION", "network":"LOCAL", "llm_capable": True},
    "M": {"title":"REPORTS (XLSX + HTML)", "network":"LOCAL", "llm_capable": True},
}
...
def _print_run_banner(args) -> None:
    ...
    llm_any = bool(args.use_llm_d or args.use_llm_h or args.use_llm_l or args.use_llm_m)
    print(
        "LLM: "
        + ("ON" if llm_any else "OFF")
        + f"  (D={args.use_llm_d}, H={args.use_llm_h}, L={args.use_llm_l}, M={args.use_llm_m})"
    )
    if llm_any:
        print(f"  provider={args.llm_provider} model={args.llm_model} base={args.llm_api_base or '(default)'}")
        if args.use_llm_d:
            print(f"  D: extra_per_cluster={args.llm_extra_per_cluster} max_tokens={args.llm_max_tokens_d}")
        if args.use_llm_h:
            print(f"  H: borderline_cap={args.llm_borderline_cap} max_tokens={args.llm_max_tokens_h}")
        if args.use_llm_l:
            print(f"  L: max_tokens={args.llm_max_tokens_l}")
        if args.use_llm_m:
            print(f"  M: max_tokens={args.llm_max_tokens_m}")
    print()
def _print_stage_banner(stage: str, args) -> None:
    ...
    llm_on = False
    if stage == "D" and args.use_llm_d:
        llm_on = True
    if stage == "H" and args.use_llm_h:
        llm_on = True
...
def _maybe_pause_before_llm(args, stages: List[str]) -> None:
    ...
    for i, st in enumerate(stages):
        if (st == "D" and args.use_llm_d) or (st == "H" and args.use_llm_h) or (st == "L" and args.use_llm_l) or (st == "M" and args.use_llm_m):
            llm_stage_idx = i
            break
    ...
    print("\n[runner] Next is an LLM-enabled stage. Turn VPN ON now if needed.")
    ans = input("Continue? [y/N]: ").strip().lower()
    if ans not in ("y", "yes"):
        raise SystemExit("Stopped by user.")


def _llm_enabled_for_stage(stage: str, args) -> bool:
    if stage == "D":
        return bool(getattr(args, "use_llm_d", False))
    if stage == "H":
        return bool(getattr(args, "use_llm_h", False))
    if stage == "L":
        return bool(getattr(args, "use_llm_l", False))
    if stage == "M":
        return bool(getattr(args, "use_llm_m", False))
    return False

def _vpn_hint_for_stage(stage: str, args) -> str:
    inf = STAGE_INFO.get(stage, {"network": "LOCAL", "llm_capable": False})
    net = inf.get("network", "LOCAL")
    if net == "WB":
        return "OFF (WB stage)"
    if _llm_enabled_for_stage(stage, args):
        return "ON (LLM stage)"
    return "doesn't matter (local-only)"

def pause_between_stages(pause: bool, *, current_stage: str, next_stage: Optional[str], args) -> None:
    """Optional interactive pause between stages with VPN hints (like wb_revive.py)."""
    if not pause or not next_stage:
        return
    if getattr(args, "yes", False):
        print(f"[runner] pause_between_stages: auto-yes -> next [{next_stage}]")
        return
    if not sys.stdin.isatty():
        return

    nxt_inf = STAGE_INFO.get(next_stage, {"title": "", "network": "", "llm_capable": False})
    vpn = _vpn_hint_for_stage(next_stage, args)
    llm_on = _llm_enabled_for_stage(next_stage, args)
    net = nxt_inf.get("network", "")

    print("\n--- Pause between stages ---")
    print(f"Next: [{next_stage}] {nxt_inf.get('title','')}")
    print(f"Network: {net}")
    print(f"VPN hint: {vpn}")
    if next_stage in ("D","H","L","M"):
        print(f"LLM: {'ON' if llm_on else 'OFF'}")
    ans = input("Continue? [y/N]: ").strip().lower()
    if ans not in ("y", "yes"):
        raise SystemExit("Stopped by user.")
...
def _yn(question: str, default: bool = False) -> bool:
    ...
    if not ans:
        return default
    return ans in ("y", "yes", "1", "да", "д")

def interactive_menu() -> None:
    """Interactive runner menu (requested: like wb_revive.py)."""
    ap = build_cli_parser()
    base = ap.parse_args([])
    ...
    while True:
        print("\nРежимы:")
        print("  1) Полный прогон A..M (rules-only, без LLM)")
        print("  2) Полный прогон A..M (LLM только Stage L+M: формулировки/exec summary)")
        print("  3) Полный прогон A..M (LLM D+H+L+M, с паузами между стадиями для VPN)")
        print("  4) Пользовательский прогон (выбор start/end, флаги LLM)")
        print("  5) Настройки .env (LLM_PROVIDER/LLM_MODEL/ключи)")
        print("  6) Показать стадии A..M")
        print("  7) Выход")

        choice = input("Номер: ").strip().lower() if sys.stdin.isatty() else "7"
        ...
        if choice not in ("1","2","3","4","5"):
            print("Нужно 1..7 (7=выход).") 
            continue

        if choice == "5":
            env = read_dotenv(env_path)
            provider = env.get("LLM_PROVIDER", os.getenv("LLM_PROVIDER","openrouter")).strip() or "openrouter"
            model = env.get("LLM_MODEL", os.getenv("LLM_MODEL","openai/gpt-4o-mini")).strip() or "openai/gpt-4o-mini"
            api_base = env.get("LLM_API_BASE", os.getenv("LLM_API_BASE","")).strip()

            print("\nТекущие .env настройки (если файла нет, будет создан):")
            print(f"  LLM_PROVIDER={provider}")
            print(f"  LLM_MODEL={model}")
            print(f"  LLM_API_BASE={api_base}")
            provider = _prompt_str("LLM_PROVIDER (openrouter/openai)", provider).lower()
            model = _prompt_str("LLM_MODEL", model)
            api_base = _prompt_str("LLM_API_BASE (optional)", api_base)

            updates = {"LLM_PROVIDER": provider, "LLM_MODEL": model, "LLM_API_BASE": api_base}

            if provider == "openai":
                if _yn("Обновить OPENAI_API_KEY?", default=False):
                    updates["OPENAI_API_KEY"] = _prompt_str("OPENAI_API_KEY", env.get("OPENAI_API_KEY", ""))
            else:
                if _yn("Обновить OPENROUTER_API_KEY?", default=False):
                    updates["OPENROUTER_API_KEY"] = _prompt_str("OPENROUTER_API_KEY", env.get("OPENROUTER_API_KEY", ""))

            write_dotenv(env_path, updates)
            load_env_file(env_path, override=True)
            print("Сохранено. .env загружен в окружение.")
            continue

        # Common run settings
        args = ap.parse_args([])  # fresh defaults
        args.out = _prompt_str("Папка вывода (--out)", getattr(base, "out", "RUN_TEST") or "RUN_TEST")
        args.input = _prompt_str("Input xlsx (--input, пусто=авто)", getattr(base, "input", ""))
        args.sheet = _prompt_str("Sheet (--sheet)", getattr(base, "sheet", "INPUT_64"))
        args.resume = _yn("Resume (пропускать уже обработанные nm_id)?", default=True)
        args.dedupe = _yn("Dedupe scope?", default=True)
        args.pause_before_llm = False
        args.pause_between_stages = False

        # Defaults per mode
        if choice == "1":
            args.start_stage, args.end_stage = "A", "M"
            args.use_llm_d = args.use_llm_h = args.use_llm_l = args.use_llm_m = False

        elif choice == "2":
            args.start_stage, args.end_stage = "A", "M"
            args.use_llm_d = args.use_llm_h = False
            args.use_llm_l = True
            args.use_llm_m = True
            args.pause_before_llm = _yn("Пауза перед первой LLM стадией (VPN подсказка)?", default=True)

        elif choice == "3":
            args.start_stage, args.end_stage = "A", "M"
            args.use_llm_d = args.use_llm_h = args.use_llm_l = args.use_llm_m = True
            args.pause_between_stages = _yn("Пауза МЕЖДУ стадиями (чтобы включать/выключать VPN по подсказкам)?", default=True)

        elif choice == "4":
            args.start_stage = _prompt_str("Start stage (A..M)", "A").upper()
            args.end_stage = _prompt_str("End stage (A..M)", "M").upper()
            args.use_llm_d = _yn("LLM в Stage D (enrich queries)?", default=False)
            args.use_llm_h = _yn("LLM в Stage H (borderline relevance)?", default=False)
            args.use_llm_l = _yn("LLM в Stage L (формулировка)?", default=True)
            args.use_llm_m = _yn("LLM в Stage M (exec summary)?", default=True)
            args.pause_before_llm = _yn("Пауза перед первой LLM стадией?", default=True)
            args.pause_between_stages = _yn("Пауза между стадиями?", default=False)
...
def build_cli_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--menu", action="store_true", help="Interactive menu (no-args also opens menu)")
    ap.add_argument("--out", default="", help="Output directory (created if missing). Example: RUN_TEST")
    ...
    ap.add_argument("--pause_between_stages", action="store_true", help="Pause between stages (VPN/LLM hints each time)")
    ...
    # LLM toggles per stage
    ap.add_argument("--use_llm_d", action="store_true", help="Use LLM in Stage D (query enrichment; strict validation)")
    ap.add_argument("--use_llm_h", action="store_true", help="Use LLM in Stage H (borderline relevance only)")
    ...
    # Stage-specific LLM budgets
    ap.add_argument("--llm_max_tokens_d", type=int, default=900, help="Stage D max tokens")
    ap.add_argument("--llm_max_tokens_h", type=int, default=900, help="Stage H max tokens")
    ...
    # Stage D knobs
    ap.add_argument("--llm_extra_per_cluster", type=int, default=3, help="Stage D: extra LLM queries per cluster")
    ap.add_argument("--rules_per_cluster", type=int, default=8, help="Stage D: rules queries per cluster (cap)")

    # Stage H knobs
    ap.add_argument("--llm_borderline_cap", type=int, default=18, help="Stage H: max borderline items sent to LLM per cluster")
...
def run_selected_stages(args) -> None:
    ...
    if not args.out or not str(args.out).strip():
        raise SystemExit("--out is required (unless you use --list_stages)")
    ...
    def _get_api_key() -> str:
        nonlocal api_key_cache
        if api_key_cache is None:
            api_key_cache = _resolve_api_key(args.llm_provider, args.llm_api_key)
    ...
    for i, st in enumerate(stages):
        ...
        if st == "A":
            ...
            api_key = _get_api_key() if args.use_llm_h else ""
            stage_H_relevance_filter(
                out_dir=out_dir,
                resume=args.resume,
                strict=args.strict,
                use_llm_h=args.use_llm_h,
                llm_provider=args.llm_provider,
                llm_model=args.llm_model,
                api_key=api_key,
                llm_base_url=args.llm_api_base,
                ...
                llm_temperature=args.llm_temperature,
                borderline_cap=args.llm_borderline_cap,
            )
            ...
            api_key = _get_api_key() if args.use_llm_l else _resolve_api_key(args.llm_provider, args.llm_api_key)
            ...
            api_key = _get_api_key() if args.use_llm_m else _resolve_api_key(args.llm_provider, args.llm_api_key)
            stage_M_reports(
                out_dir=out_dir,
                resume=args.resume,
        ...
        pause_between_stages(getattr(args, "pause_between_stages", False), current_stage=st, next_stage=next_stage, args=args)

    print("\nDONE. Output dir:", str(out_dir.resolve()))
def main() -> None:
    # If launched with no args, open interactive menu (like wb_revive.py)
    if len(sys.argv) == 1:
        interactive_menu()
        return

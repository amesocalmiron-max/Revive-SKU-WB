...
SCRIPT_VERSION = "2.0.8-rewrite-2026-02-22-menufix9"
SCHEMA_VERSION = "2.0.8-rewrite-2026-02-22-menufix9"
...
def stage_sku(code: str, nm_id: str, msg: str, level: int = 2, err: bool = False) -> None:
    stage_line(code, f"nm={nm_id} {msg}", level=level, err=err)
...
def stage_line(code: str, msg: str, level: int = 1, err: bool = False) -> None:
    _log_to_files(code, msg, level, err=err)
    if not v(level):
        return
...
def parse_search_items(js: dict) -> List[dict]:
    ...
    for idx, p in enumerate(products, start=1):
        ...
        if not nm.isdigit():
            continue
        imt = safe_int(p.get("root") or p.get("imtId") or p.get("rootId"))
...
def stage_G_lite(out_dir: Path, *, timeout: int, sleep_s: float, resume: bool) -> Path:
    ...
    for rec in iter_jsonl(pool_path):
        ...
        for cluster in CLUSTERS:
            ...
            out_clusters.append({"cluster": cluster, "items": lite_items})
...
def stage_I_pulse(out_dir: Path, *, timeout: int, sleep_s: float, resume: bool, strict: bool = False) -> Path:
    ...
    base_urls = list(reviews_cfg.get("base_urls") or DEFAULT_REVIEWS_CFG["base_urls"])
    ...
    take = int(reviews_cfg.get("take") or 30)
    max_pages = int(reviews_cfg.get("max_pages") or 40)
    max_skip = int(reviews_cfg.get("max_skip") or 1000)
